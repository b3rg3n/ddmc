...
init python: 
    ...
    with renpy.file("advanced_scripts/poemwords.txt") as pf:
        for line in pf:
...
label poem(transition=True):
    ...
    if persistent.playthrough == 3: 
        ...
        if persistent.playthrough == 0 and chapter == 0:
            call screen dialog("Самое время написать тексты!\n\nВыберите слова, которые, как вам кажется,\nпонравятся определённой девушке.\n\nЭто абсолютно никак не повлияет на историю,\nно отсутствие мини-игры бы попросту было странным!\n\n(Настолько не повлияет на историю,\nчто переводчики даже не удосужились перевести\nслова, которые понравятся конкретной девочке.)\n\nВсё равно вне зависимости от выборов,\nЦГ получит одна и та же девочка.", ok_action=Return())
        if persistent.playthrough == 2 and chapter == 2:
